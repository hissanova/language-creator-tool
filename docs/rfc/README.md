# RFC Index

RFCs record proposals and non-obvious or cross-cutting design decisions. Each document's own status is authoritative: **Draft** means proposed and not accepted, **Accepted** means adopted as a design decision, and **Superseded** means replaced by a later decision.

| RFC | Title | Status |
| --- | --- | --- |
| [0001](0001-section-blocks.md) | Section Blocks | Draft |
| [0002](0002-formedtext-selectors-provenance.md) | FormedText, Target Selectors, and Annotation Provenance | Draft |
| [0003](0003-core-textnode-selector-ref-transform.md) | TextNode, SelectorNode, Ref, and Transform Core Model | Accepted |
| [0004](0004-functional-core-imperative-shell.md) | Functional Core, Imperative Shell | Accepted |
